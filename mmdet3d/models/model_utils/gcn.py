...
class TreeGCN(nn.Module):
    def __init__(self, depth, features, degrees, support=10, node=1, upsample=False, activation=True,loop_non_linear=False):
        ...
        if self.upsample:
            # shape (node, in_feature, out_feature)[96, 256, 256, 256, 128, 128, 128, 3]
    ...
    def forward(self, tree):
        ...
        for inx in range(self.depth+1):
            root_num = tree[inx].size(1)#2
            repeat_num = int(self.node / root_num)#1
            root_node = self.W_root[inx](tree[inx])# 0 B*2*192
            root = root + root_node.repeat(1,1,repeat_num).view(batch_size,-1,self.out_feature)  #B*2*128
            # after reshape, for node = 2, 
        branch = 0
        if self.upsample:
            branch = tree[-1].unsqueeze(2) @ self.W_branch #B*1*1*96-》192
