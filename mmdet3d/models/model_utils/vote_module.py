...
class VoteModule(nn.Module):
    ...
    def forward(self, seed_points, seed_feats):
        ...
        votes = self.conv_out(x)#8*259*1024

        votes = votes.transpose(2, 1).view(batch_size, num_seed,
                                           self.vote_per_seed, -1)#8*1024*1*259偏移+feature
        ...
        if self.vote_xyz_range is not None:
            ...
            vote_points = (seed_points.unsqueeze(2) + offset).contiguous()#seed_points+vote_offset
        vote_points = vote_points.view(batch_size, num_vote, 3)
        offset = offset.reshape(batch_size, num_vote, 3).transpose(2, 1)#8*3*1024
