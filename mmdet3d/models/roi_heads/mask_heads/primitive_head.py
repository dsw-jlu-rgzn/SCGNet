...
from mmdet3d.models.model_utils import GraphRelationModule
...
class PrimitiveHead(BaseModule):
    ...
    def forward(self, feats_dict, sample_mod):
        ...
        vote_aggregation_ret = self.vote_aggregation(vote_points,
                                                     vote_features,
                                                     sample_indices)
        aggregated_points, features, aggregated_indices = vote_aggregation_ret#3*128*1024
        ...
        if self.graph_relation_module:
            relation_features, cls_scores = self.graph_relation_module(features)
            results['first_sem_scores_'+self.primitive_mode] = cls_scores.contiguous()
    ...
    def loss(self,
        ...
        # ADD first semantic loss
        if self.first_semantic_loss:
            losses['first_sem_loss_'+self.primitive_mode] = self.first_semantic_loss(
                bbox_preds['first_sem_scores_'+self.primitive_mode].transpose(2, 1),
                gt_sem_cls_label,
                weight=gt_primitive_mask)
        return losses
