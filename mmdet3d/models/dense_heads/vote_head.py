...
from mmdet3d.models.model_utils import VoteModule, DisARM, GraphRelationModule, GlobalModule
...
class VoteHead(BaseModule):
    ...
    def __init__(self,
                 ...
                 disarm_module_cfg=None,
                 graph_relation_module_cfg=None,
                 global_module_cfg=None,
                 pred_layer_cfg=None,
                 conv_cfg=dict(type='Conv1d'),
                 norm_cfg=dict(type='BN1d'),
                 relation_anchor_loss=None,
                 ...
                 semantic_loss=None,
                 iou_loss=None,
                 feature_center_loss=None,
                 only_feature_center_loss=None,
                 init_cfg=None,
                 ):
        ...
        if first_semantic_loss is not None:
            ...
            self.first_semantic_loss = None
        if only_feature_center_loss is not None:
            self.only_feature_center_loss = build_loss(only_feature_center_loss)
            self.only_feature_center = nn.Parameter(torch.randn(self.num_classes, 128))
        else:
            self.only_feature_center_loss = None
        if feature_center_loss is not None:
            self.feature_center_loss = build_loss(feature_center_loss)
        else:
            self.feature_center_loss = None

            #self.semantic_center = nn.Parameter(torch.randn(self.num_classes, ))
        if relation_anchor_loss is not None:
            self.relation_anchor_loss = build_loss(relation_anchor_loss)
        else:
            self.relation_anchor_loss = None
        ...
        # Relation Module
        if not disarm_module_cfg:
            self.disarm_module = None
        else:
            self.disarm_module = DisARM(**disarm_module_cfg)
        if not graph_relation_module_cfg:
            self.graph_relation_module = None
        else:
            self.graph_relation_module = GraphRelationModule(**graph_relation_module_cfg)
        if not global_module_cfg:
            self.global_module = None
        else:
            self.global_module = GlobalModule(**global_module_cfg)
    ...
    def forward(self, feat_dict, sample_mod):
        ...
        # ADD RELATION CONTEXT HERE
        if self.disarm_module:
            relation_features = self.disarm_module(aggregated_points, features)
            results.update(relation_features)
            # add relation_features and features
            features = relation_features['relation'] + features
        if self.graph_relation_module:
            relation_features, cls_scores = self.graph_relation_module(features, aggregated_points)
            results['first_sem_scores'] = cls_scores.contiguous()
            features = relation_features + features
            results['fusion_features'] = features
            results['feature_center'] = self.graph_relation_module.feature_center
            if self.global_module:
                features = features + self.global_module(seed_features, relation_features, features)
        if self.only_feature_center_loss:
            results['only_feature_center'] = self.only_feature_center
        # 3. predict bbox and score
    ...
    def loss(self,
        ...
        if self.first_semantic_loss:
            # calculate semantic loss
            first_semantic_loss = self.first_semantic_loss(
                bbox_preds['first_sem_scores'].transpose(2, 1),
                mask_targets,
                weight=box_loss_weights)
            losses['first_semantic_loss'] = first_semantic_loss
        if self.only_feature_center_loss:
            # calculate feature center loss
            only_feature_center_loss = self.only_feature_center_loss(
                bbox_preds['aggregated_features'].transpose(2, 1),#8*256*128
                mask_targets,
                centers=bbox_preds['only_feature_center'],
                weight=box_loss_weights)
            losses['only_feature_center_loss'] = only_feature_center_loss
        if self.feature_center_loss:
            # calculate feature center loss
            feature_center_loss = self.feature_center_loss(
                bbox_preds['aggregated_features'].transpose(2, 1),#8*256*128
                mask_targets,
                centers=bbox_preds['feature_center'],
                weight=box_loss_weights)


            losses['feature_center_loss'] = feature_center_loss
        if self.relation_anchor_loss:
            relation_anchor_loss = self.relation_anchor_loss(
                bbox_preds['rn_anchor_objectness'],
                objectness_targets,
                weight=objectness_weights
            )
            losses['relation_anchor_loss'] = relation_anchor_loss
    ...
    def multiclass_nms_single(self, obj_scores, sem_scores, bbox, points,
                              input_meta, return_selection=False):
