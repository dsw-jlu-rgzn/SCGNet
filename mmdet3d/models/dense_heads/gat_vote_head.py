...
from mmdet3d.models.model_utils import GAT
from ..model_utils.GAT import get_batch_graph_from_proposals, normalization_adj_batch, GAT_Batch
import torch_geometric

@HEADS.register_module()
class GATVoteHead(BaseModule):
    ...
    def __init__(self,
                 ...
                 pred_semantic_cfg=None):
        super(GATVoteHead, self).__init__(init_cfg=init_cfg)
        ...
        # self.PAM = Graph_based_module.PAM_Module(danet_channel)
        # self.CAM = Graph_based_module.CAM_Module(danet_channel)
        # Bbox classification and regression
        self.conv_pred = BaseConvBboxHead(
            **pred_layer_cfg,
            num_cls_out_channels=self._get_cls_out_channels(),
            num_reg_out_channels=self._get_reg_out_channels())
       #  self.semantic_pred = BaseConvSingleHead(
       #     **pred_semantic_cfg,
       #     num_cls_out_channels=self._get_cls_out_channels
       # )
        self.predict_center = torch.nn.Conv1d(128, 3, 1)  # +self.num_class
        self.predict_sem = torch.nn.Conv1d(128, self.num_classes, 1)
        ...
        self.GAT = GAT_Batch(128,128,128,0.4,0.2,4)
        self.concat_linear = torch.nn.Linear(128, 128)
    ...
    def forward(self, feat_dict, sample_mod):
        ...
        #1, 1.2 , 1,4, 1.6
        radius = 1.6
        semantic_features = self.semantic_pred(features)#B*18*256

        cls_prob, cls_feature = self._region_classification(features)  # b, 256, 18  | b, 18, 256

        z = self.relation_fc_1(features)  # 8*128*256
        ...
        for relation_ in relation:
            adj_ = torch_geometric.utils.to_dense_adj(relation_)
            adj.append(adj_)
        adj = torch.cat(adj,0)


        #adj = get_batch_graph_from_proposals(aggregated_points, radius)
        nor_adj = normalization_adj_batch(adj)
        features = features.transpose(1,2)#8*256*128
        GAT_net = self.GAT(represent, nor_adj)
        # net = torch.cat([GAT_net, features],2)#8*256*128 ->8*256*256
        # net = self.concat_linear(net)
        net = GAT_net + features
        net = self.concat_linear(net)
