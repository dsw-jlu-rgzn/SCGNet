...
from ..model_utils.pointgnn import GraphNetAutoCenter, batch_edge_from_points


@HEADS.register_module()
class GnnVoteHeadv2(BaseModule):
    ...
    def __init__(self,
                 ...
                 graph_net_layers=1,
                 graph_option=None):
        super(GnnVoteHeadv2, self).__init__(init_cfg=init_cfg)
        ...
        if self.graph_option is not None:
            self.concat_linear = torch.nn.Linear(256, 128)
            self.graph_nets = nn.ModuleList()
            self.graph_seed_nets = nn.ModuleList()
            self.seed_concat_linear = torch.nn.Linear(512, 256)
            for i in range(graph_net_layers):
                self.graph_nets.append(GraphNetAutoCenter())
                self.graph_seed_nets.append(GraphNetAutoCenter(auto_offset=True, auto_offset_MLP_depth_list=[256, 64, 3],
                         edge_MLP_depth_list=[256 + 3, 256, 256],
                         update_MLP_depth_list=[256, 256, 256]))
    ...
    def forward(self, feat_dict, sample_mod):
        ...
        seed_points, seed_features, seed_indices = self._extract_input(
            feat_dict)#8*1024*3     8*256*1024   8*1024
        if self.graph_option is not None:#8*256*3  8*128*256  8*256
            batch_size = seed_features.shape[0]
            the_features = torch.clone(seed_features.transpose(1, 2))#8*1024*256
            edges = batch_edge_from_points(seed_points, radius=1)
            graph_features = []
            for batch_id in range(batch_size):
                edge = edges[batch_id]
                the_feature = the_features[batch_id]#256*128
                coordinate = seed_points[batch_id]#256*3
                keypoint_indices = seed_indices[batch_id]#256
                for i, graph_net in enumerate(self.graph_seed_nets):
                    the_feature = graph_net(the_feature, coordinate, keypoint_indices, edge)#256*128
                graph_features.append(the_feature.unsqueeze(0))
            graph_features = torch.cat(graph_features, dim=0)

            seed_features = torch.cat([torch.clone(graph_features), seed_features.transpose(1,2)],2)
            seed_features = self.seed_concat_linear(seed_features).transpose(1,2)
        ...
        if self.graph_option is not None:
            ...
            the_features = torch.clone(features.transpose(1, 2))
            edges = batch_edge_from_points(aggregated_points, radius=1)
            graph_features = []
            for batch_id in range(batch_size):
                edge = edges[batch_id]
                the_feature = the_features[batch_id]#256*128
                coordinate = aggregated_points[batch_id]#256*3
                keypoint_indices = aggregated_indices[batch_id]#256
                for i, graph_net in enumerate(self.graph_nets):
                    the_feature = graph_net(the_feature, coordinate, keypoint_indices, edge)#256*128
                graph_features.append(the_feature.unsqueeze(0))
            graph_features = torch.cat(graph_features, dim=0)

            features = torch.cat([torch.clone(graph_features), features.transpose(1, 2)], 2)
            features = self.concat_linear(features).transpose(1, 2)

            #features = torch.clone(graph_features).transpose(1,2) + features#8*128*256
        results['aggregated_points'] = aggregated_points
        results['aggregated_features'] = features
        #results['aggregated_features'] = graph_features
