...
class SeedVoteHead(BaseModule):
    ...
    def __init__(self,
                 ...
                 init_cfg=None,
                 seed_loss=None):
        super(SeedVoteHead, self).__init__(init_cfg=init_cfg)
        ...
        if seed_loss is not None:
            self.seed_cls_loss = build_loss(seed_loss)
        else:
            self.seed_cls_loss = None
            # ------------------------ #
        ...
        #self.mlp_seed = torch.nn.Sequential()
        self.mlp_channel = [259, 128, 1]
        # self.mlp_seed = nn.Sequential(
        #     nn.Linear(self.mlp_channel[0], self.mlp_channel[1]),
        #     nn.ReLU(inplace=True),
        #     nn.Linear(self.mlp_channel[1], self.mlp_channel[2]),
        # )
        self.FC_layer_cla = nn.Sequential(
            nn.Conv1d(self.mlp_channel[0], self.mlp_channel[1],kernel_size=1),
            nn.BatchNorm1d(self.mlp_channel[1]),
            nn.Conv1d(self.mlp_channel[1], self.mlp_channel[2],kernel_size=1)
        )


        # for i in range(len(self.mlp_channel)-1):
        #     self.mlp_seed.add_module(
        #         f'layer{i}',
        #         ConvModule(
        #             self.mlp_channel[i],
        #             self.mlp_channel[i + 1],
        #             kernel_size=1,
        #             stride=1,
        #             conv_cfg=dict(type='Conv1d'),
        #             norm_cfg=dict(type='BN1d'),
    ...
    def forward(self, feat_dict, sample_mod):
        ...
        seed_points_tran = seed_points.permute(0, 2, 1) # 8 * 3 * 1024
        seed_before_pred = torch.cat([seed_points_tran, seed_features], 1)# 8 * 259 * 1024
        #seed_before_pred = seed_before_pred.permute(0, 2, 1)
        seed_pred = self.FC_layer_cla(seed_before_pred) #8*1*1024
        seed_score = seed_pred.sigmoid()#8*1*1024
        #seed_pred = seed_pred.permute(0, 2, 1).contiguous()#2*1024
        # 1. generate vote_points from seed_points
        vote_points, vote_features, vote_offset = self.vote_module(
            seed_points, seed_features)#8*256*1024
        if self.seed_cls_loss:
            vote_features = torch.cat([seed_score, vote_features], 1)#8*257*1024
    ...
    def loss(self,
        ...
        seed_cls_mask = torch.gather(vote_target_masks, 1, bbox_preds['seed_indices']).long()
        if self.seed_cls_loss:
            seed_cls_loss = self.seed_cls_loss(
                bbox_preds['seed_pred'],
                seed_cls_mask,
            )
            losses['seed_cls_loss'] = seed_cls_loss
        if ret_target:
