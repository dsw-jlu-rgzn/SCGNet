...
from .base_conv_bbox_head import BaseConvBboxHead, SemanticPredictHead
from mmdet3d.models.model_utils import Graph_based_module
from mmdet3d.models.model_utils import Global_residual
from mmdet3d.models.model_utils.treegan_network import Generator, Discriminator
@HEADS.register_module()
class GanVoteHead(BaseModule):
    ...
    def __init__(self,
                 ...
                 senet_channel=None,
                 danet_channel=None,
                 is_global_residual_net=None,
                 is_treegan=None,
                 G_config=None,
                 D_config=None,
                 pred_layer_cfg_gan=None,
                 no_gradient_G=True,
                 no_gradient_D=True,
                 new_class=None,
                 is_seda=None,
                 first_semantic_loss=None
                 ):
        super(GanVoteHead, self).__init__(init_cfg=init_cfg)
        self.num_classes = num_classes
        self.train_cfg = train_cfg
        self.test_cfg = test_cfg
        self.gt_per_seed = vote_module_cfg['gt_per_seed']
        self.num_proposal = vote_aggregation_cfg['num_point']
        self.is_treegan = is_treegan
        self.is_seda = is_seda
        if self.is_treegan is not None:
            self.D = Discriminator(**D_config)
            self.G =Generator(**G_config)
            self.TreeGAN_ckp_dir ="/nfs/volume-92-1/shuweidong_i/the_mmdetection3d/mmdetection3d/checkpoints/" \
                    "chairtree_ckpt_520_ShapeNet2021-09-23-12-40.pt"
            self.D.load_state_dict(torch.load(self.TreeGAN_ckp_dir)['D_state_dict'])
            self.G.load_state_dict(torch.load(self.TreeGAN_ckp_dir)['G_state_dict'])
            self.gamma_gan = nn.Parameter(torch.zeros(1))
            self.new_class = new_class
            # new_class = [4, 2, 7, 16, 17, 0, 1, 3]
            # a = [4,2,7,16,17,0,1,3]
            # b = {'table': 0, 'chair': 1, 'bookshelf': 2, 'bathtub': 3, 'garbagebin': 4, 'cabinet': 5, 'bed': 6, 'sofa': 7}
            self.tran_class = {j: i for (i, j) in enumerate(new_class)}  # [0:0,2:1,4:2]
            self.bn3 = torch.nn.BatchNorm1d(128)
            self.linear_fea = torch.nn.Linear(64, 128)
            self.linear_cat = torch.nn.Linear(256, 128)
        #设置梯度不传导
            if no_gradient_G == True:
                for param in self.G.parameters():
                    param.requires_grad = False
            if no_gradient_D ==True:
        ...
        if first_semantic_loss is not None:
            self.first_semantic_loss = build_loss(first_semantic_loss)
        ...
        self.conv_pred_semantic_first =SemanticPredictHead(
            **pred_layer_cfg_gan,
            num_cls_out_channels=self._get_cls_out_channels_gan()
        ...
        self.is_global_residual_net = is_global_residual_net
        if self.is_global_residual_net is not None:
            self.GlobalResidualNet = Global_residual.GlobalResidualNet()




    def _get_cls_out_channels_gan(self):
    ...
    def _predict_cGAN_labels_z(self, proposals):#proposals:8*128*256
        '''return the predicted_gan_lalbeled the GAN need'''
        predict_cls, proposals_z = self.conv_pred_semantic_first(proposals)#B*128*256 ->B*18*256, B*256*96
        #print(predict_cls.shape, proposals_z.shape)
        #predicted = torch.max(output.data, 1)[1]
        predict_cls_trans = predict_cls.transpose(2,1)#8*256*18
        proposals_label =torch.max(predict_cls, 1)[1]#B*256
        return proposals_label, predict_cls_trans, proposals_z


    def generate_GAN_object_test(self, predicted_label, proposals_z):#B*256   b*256*96
        the_sem_cls_label = predicted_label.reshape(-1, )  # 8*256
        the_device = the_sem_cls_label.device
        change_sem_mask = torch.tensor(
            [1 if i.item() in self.new_class else 0 for i in the_sem_cls_label])  # 8*256 mask->2048

        # mask_index = torch.arange(256)[]
        mask_index = torch.LongTensor([i for i, j in enumerate(change_sem_mask) if j.item() == 1])  # 掩码的index
        mask_labels = torch.LongTensor(
            [self.tran_class[j.item()] for j in the_sem_cls_label[mask_index]]).to(
            the_device)  # 从the_sem_cls_label(40类别)->treegan的类别
        len_mask_index = mask_index.shape[0]

        len_all_index = change_sem_mask.shape[0]
        # proposals_z[mask_labels], mask_labels, class_num
        one_hot_label = F.one_hot(mask_labels, len(self.new_class))  # 800*8需要变成cuda
        proposals_z = proposals_z.reshape(len_all_index, -1)  # 2048*96#
        # proposals_z[mask_index][:,:len(self.new_class)] = one_hot_label#为什么这样子写会出bug
        prepared_z = proposals_z[mask_index]
        prepared_z[:, :len(self.new_class)] = one_hot_label
        prepared_z = prepared_z.reshape(-1, 1, 96)
        # 16-6061 32-5633 48-7731 64-9800
        batchsize = 24  # 16-6061 32-5633 48-7731 64-9800
        with torch.no_grad():
            G_all_instance = []
            for i in range(0, len(prepared_z), batchsize):
                begin = i
                end = i + batchsize
                new_prepared_z = prepared_z[begin:end].reshape(-1, 1, 96)

                G_new_instance = self.G([new_prepared_z])
                G_all_instance.append(G_new_instance)
            G_instance = torch.cat(G_all_instance, 0)

        return mask_labels, mask_index, G_instance




    def generate_GAN_feature(self, proposals, predicted_label, proposals_z ):
        the_sem_cls_label = predicted_label.reshape(-1, )#8*256
        the_device = the_sem_cls_label.device
        change_sem_mask = torch.tensor(
            [1 if i.item() in self.new_class else 0 for i in the_sem_cls_label])  # 8*256 mask->2048
        len_mask_index =torch.sum(change_sem_mask)
        if len_mask_index == 0:
            return proposals
        else:
            #mask_index = torch.arange(256)[]
            mask_index = torch.LongTensor([i for i, j in enumerate(change_sem_mask) if j.item() == 1])  # 掩码的index
            mask_labels = torch.LongTensor(
                [self.tran_class[j.item()] for j in the_sem_cls_label[mask_index]]).to(the_device)  # 从the_sem_cls_label(40类别)->treegan的类别
            len_mask_index = mask_index.shape[0]
            ...
            prepared_z = prepared_z.reshape(-1, 1, 96)
            # ----------------------------------- #
            batchsize = 48#16-6061 32-5633 48-7731 64-9800
            with torch.no_grad():
                G_all_instance = []
                for i in range(0,len(prepared_z),batchsize):
                    begin = i
                    ...
                    G_all_instance.append(G_new_instance)
                G_instance = torch.cat(G_all_instance, 0)
                D_features = self.D.get_feature(G_instance)
            #------------------------------------------- #
            # with torch.no_grad():
            #     G_all_instance = []
            #     for i in range(len(prepared_z)):
            #         new_prepared_z = prepared_z[i].reshape(1,1,96)
            #         G_new_instance = self.G([new_prepared_z])
            #         G_all_instance.append(G_new_instance)
            #     G_instance = torch.cat(G_all_instance, 0)
            #     D_features = self.D.get_feature(G_instance)
                # ---------------------------------------- #
            # with torch.no_grad():
            #     G_instance = self.G([prepared_z])
            #     D_features = self.D.get_feature(G_instance)#ALL_batch*64
            tran_D_features = F.relu(self.bn3(self.linear_fea(D_features)))
            proposals = proposals.transpose(1,2).reshape(-1,128)#8*128*256 -> 8*256*128 -> 2048*128
            fused_proposals = torch.clone(proposals)
            tran_D_features = torch.cat([proposals[mask_index], self.gamma_gan * tran_D_features], 1)
            tran_D_features = self.linear_cat(tran_D_features)
            fused_proposals[mask_index] = tran_D_features
            #8*256, 128 ->8, 128, 256
            return fused_proposals.reshape(-1,256,128).transpose(1,2)
    ...
    def forward(self, feat_dict, sample_mod):
        ...
        aggregated_points, features, aggregated_indices = vote_aggregation_ret#8*256*3  8*128*256  8*256
        # --------------GAN and D-------------------------#
        if self.is_treegan is not None:
            proposal_label, predict_cls_trans, proposals_z = self._predict_cGAN_labels_z(proposals=features)#B*256, B*18*256, B*256*96
            results['proposal_label'] = proposal_label
            results['proposals_z'] = proposals_z
            results['first_sem_scores'] = predict_cls_trans
            new_features = self.generate_GAN_feature(proposals=features, predicted_label=proposal_label, proposals_z=proposals_z)
        else:
            proposal_label, predict_cls_trans, proposals_z = self._predict_cGAN_labels_z(proposals=features)
            results['first_sem_scores'] = predict_cls_trans
            new_features = features
        # ---------------------------------------#
        if self.is_seda:
            feature_dim = features.shape[1]#128
            batch_size = features.shape[0]
            net = self.SENet(new_features)
            #net = self.DANet(net)
        else:
            net = new_features

        if self.is_global_residual_net:
            #features是之前没有经过senet的特征， net是经过了senet的特征，seed_points是提取出来的特征
            net = net.contiguous().view(batch_size,feature_dim,self.num_proposal)
            new_features = new_features.contiguous().view(batch_size, feature_dim, self.num_proposal)
            net = self.GlobalResidualNet(new_features,seed_features,net)
        results['aggregated_points'] = aggregated_points#8*256*3
        results['aggregated_features'] = net#8*128*256
        results['aggregated_indices'] = aggregated_indices#8*1024*3
        # 3.0 new->SENet+DANet

        # 3. predict bbox and score
        cls_predictions, reg_predictions = self.conv_pred(net)
    ...
    def loss(self,
        ...
        semantic_loss = self.semantic_loss(#1*256*18   1*256
        ...
        # calculate semantic loss
            first_semantic_loss = self.first_semantic_loss(bbox_preds['first_sem_scores'].transpose(2, 1),
                                                       mask_targets,
                                                       weight=box_loss_weights)
            losses['first_semantic_loss'] = first_semantic_loss
        if self.iou_loss:
    ...
    def get_gan_and_bboxes(self,
        ...
        if use_nms:
            ...
            results = list()
            for b in range(batch_size):
                bbox_selected, score_selected, labels, select = \
                    self.multiclass_nms_single_index(obj_scores[b], sem_scores[b],
                                               bbox3d[b], points[b, ..., :3],
                                               input_metas[b])
                bbox = input_metas[b]['box_type_3d'](
                    bbox_selected,
                    box_dim=bbox_selected.shape[-1],
                    with_yaw=self.bbox_coder.with_rot)
                results.append((bbox, score_selected, labels, select))
    ...
    def multiclass_nms_single_index(self, obj_scores, sem_scores, bbox, points,
                              input_meta):
        """Multi-class nms in single batch.
        ...
        Returns:
            tuple[torch.Tensor]: Bounding boxes, scores and labels.and index
        """
        bbox = input_meta['box_type_3d'](
            bbox,
            box_dim=bbox.shape[-1],
        ...
        return bbox_selected, score_selected, labels, selected
