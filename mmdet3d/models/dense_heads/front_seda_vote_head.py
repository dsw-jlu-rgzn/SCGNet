...
from ..model_utils.CGNL import SpatialCGNL


@HEADS.register_module()
class FVoteHead(BaseModule):
    ...
    def __init__(self,
        ...
        super(FVoteHead, self).__init__(init_cfg=init_cfg)
        ...
        self.concat_linear = torch.nn.Linear(512, 256)
        self.sa0 = SpatialCGNL(256, int(256 / 2), use_scale=False, groups=4)
    ...
    def forward(self, feat_dict, sample_mod):
        ...
        #new_features = self.SENet(seed_features)
        # new_features = self.DANet(seed_features)#
        # seed_features = torch.cat([torch.clone(new_features), seed_features],1)#8*512*1024
        # seed_features = self.concat_linear(seed_features.transpose(1,2)).transpose(1,2)
        original_seed_features = seed_features
        feature_dim = seed_features.shape[1]
        batch_size = seed_features.shape[0]
        net1 = seed_features.view(batch_size, feature_dim, 32, 32)
        net1 = self.sa0(net1)
        seed_features = net1.view(batch_size, feature_dim, 1024)
        ...
        #net = self.SENet(features)
        features = features.contiguous().view(batch_size, feature_dim, 16, 16)
        #net = features
        #net = self.DANet(features)
        net = self.sa1(features)
        net = self.sa2(net)
        net = net.contiguous().view(batch_size, feature_dim, 256)


        # --------------------------- #
        #net = net.contiguous().view(batch_size, feature_dim, self.num_proposal)
        features = features.contiguous().view(batch_size, feature_dim, 256)

        global_features_2 = F.max_pool1d(features, kernel_size=features.size(2))  # (B, 128, 1)
        global_features_1 = F.max_pool1d(original_seed_features, kernel_size=original_seed_features.size(2))  # (B, 256, 1)
        ...
        #     net = self.GlobalResidualNet(features,seed_features,net)
