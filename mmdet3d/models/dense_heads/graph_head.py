...
from .base_conv_bbox_head import BaseConvBboxHead,  SemanticPredictHead, BaseConvSingleHead
from ..model_utils.graph import GAT, RelationFeatureFusion
'''
1. 新建一个分类器
2. 增加分类loss
3. 加入GAT
...
class GraphHead(BaseModule):
    ...
    def __init__(self,
                 ...
                 pred_sem_layer_cfg=None):
        super(GraphHead, self).__init__(init_cfg=init_cfg)
        ...
        self.conv_pred_semantic_first = BaseConvSingleHead(
            **pred_sem_layer_cfg,#8*18*256
            num_cls_out_channels=self._get_cls_out_channels_gcn())
        ...
        self.concat_conv = nn.Conv1d(256, 128, 1)
        ...
        self.relation_mlp = RelationFeatureFusion(in_channels=256)

    def _get_cls_out_channels_gcn(self):
    ...
    def forward(self, feat_dict, sample_mod):
        ...
        ### ------------------------------  ###
        cls_scores = self.conv_pred_semantic_first(features) #8*128*256 -> 8*18*256 ->8*256*18
        ...
        relation[:, 1] = torch.Tensor(list(range(256)) * 16).unsqueeze(0).repeat(batch_size, 1)
        ...
        #features = F.relu(self.bn_concat(self.concat_conv(features)))
        features = self.relation_mlp(new_features) + features
        # 3. predict bbox and score
    ...
    def loss(self,
        ...
        if self.first_semantic_loss:
            # calculate semantic loss
            first_semantic_loss = self.semantic_loss(
