...
from ..model_utils.GAT import get_batch_graph_from_proposals, normalization_adj_batch, GAT_Batch, GraphAttentionLayer, GraphContextualNetwork


@HEADS.register_module()
class GATVoteHeadV2(BaseModule):
    ...
    def __init__(self,
        ...
        super(GATVoteHeadV2, self).__init__(init_cfg=init_cfg)
        ...
        self.GAT_layer = GraphAttentionLayer(in_features=128, out_features=128, dropout=0.4, alpha=0.2)
        self.concat_linear = torch.nn.Linear(256, 128)
        self.GCN = GraphContextualNetwork(senet_channel=128, danet_channel=128, nfeat=128, nhid=128, nclass=128, dropout=0.4, alpha=0.2, nheads=4, radius=1  )
    ...
    def forward(self, feat_dict, sample_mod):
        ...
        net = self.GCN(features, aggregated_points)
        # if self.is_global_residual_net is not None:
