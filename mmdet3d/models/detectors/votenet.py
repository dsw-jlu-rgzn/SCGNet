...
class VoteNet(SingleStage3DDetector):
    ...
    def forward_train(self,
        ...
        return losses
    def get_results(self, points, img_metas=None, imgs=None, rescale=False):
        points_cat = torch.stack(points)

        x = self.extract_feat(points_cat)
        bbox_preds = self.bbox_head(x, self.test_cfg.sample_mod)
        bbox_list = self.bbox_head.get_gan_and_bboxes(
            points_cat, bbox_preds, img_metas, rescale=rescale)
        bbox_results = [
            (bbox3d2result(bboxes, scores, labels), select)
            for bboxes, scores, labels, select in bbox_list
        ]
        gan_result = self.bbox_head.generate_GAN_object_test(bbox_preds['proposal_label'], bbox_preds['proposals_z'])


        # return bbox_results
        return bbox_results,gan_result
